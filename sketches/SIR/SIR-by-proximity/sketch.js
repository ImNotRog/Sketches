...
function setup() {
    ...
    Node = function (param1, param2) {
        ...
        this.presetInfection = function () {
            ...
            // Makes sure the probability of getting the disease is 0 before each frame

            // Sets the current probability to 0
            this.currProbability = 0;
        ...
        this.giveInfection = function () {
            ...
            if (this.status == "i" || this.status == "c" || this.status == "w") {
                ...
                for (var i = 0; i < simulation.nodes.length; i++) {
                    ...
                    if (simulation.nodes[i] != this && simulation.nodes[i].status == "s") {

                        // If the distance is reasonable, then increase the probability of the Node
                        var distBetween = dist(this.pos.x, this.pos.y, simulation.nodes[i].pos.x, simulation.nodes[i].pos.y);

                        if (distBetween < simulation.infectionRadius) {
                            simulation.nodes[i].currProbability += simulation.infectionProb;
                        }
                    }
        ...
        this.calcInfection = function () {
            ...
            // Just sees if a random value is smaller than the probability
            if (random() < this.currProbability) {
                if (this.status == "s") {
                    this.status = "c";
                }
            }
    ...
    Simulation = function (x, y) {
        ...
        this.infectionProb = 0.0035;
        this.infectionLength = 64 * 5;
        this.incubationLow = 64 * 2;
        ...
        // Spawn Object
        this.spawnObject = [
            { size: 400, left: 200, top: 0, bottom: 400, right: 600 },
        ]

        // Field constants
        this.fieldArea = { left: 200, top: 0, bottom: 400, right: 600 };

        // Quarantine constants
        this.quarantineArea = { left: 0, top: 250, bottom: 400, right: 150 };
        this.quarantineLength = 32;

        // Custom Barriers
        this.sides = [new Side(0, 0, 0, 400), new Side(0, 0, 600, 0), new Side(0, 400, 600, 400), new Side(600, 0, 600, 400), new Side(150, 250, 150, 400), new Side(0, 250, 150, 250), new Side(200, 0, 200, 400)];
